...
This script is provided for convenience in case you wish to modify the ways
in which files are transformed for agent consumption from the parsed documents.
The pre-transformed files are available on Hugging Face — see corpus_scripts/README.md
for download instructions.

Input:  <data-dir>/treasury_bulletins_parsed/jsons/*.json
...
Usage:
    python transform_files_page_level.py --data-dir ~/officeqa-corpus
    python transform_files_page_level.py --data-dir ~/officeqa-corpus --file treasury_bulletin_1939_01.json
    python transform_files_page_level.py --data-dir ~/officeqa-corpus --split-files
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Convert JSON files to Markdown (multi-line tables, page-level).")
    parser.add_argument("--data-dir", type=str, required=True, help="Path to root of downloaded HF corpus (e.g. ~/officeqa-corpus)")
    ...
    input_dir = os.path.join(args.data_dir, "treasury_bulletins_parsed", "jsons")
    output_dir = os.path.join(args.data_dir, "treasury_bulletins_parsed", "transformed_page_level")
